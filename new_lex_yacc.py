...
def p_field_declartaion(p):
    ...
    var = str(p[3])
    if var.isalnum():
        global_symbol_table[var] = sym_table(var_name = var, var_type = str(p[2]), scope_num = actual_scope)
...
def p_method_header(p):
    ...
    if len(p) == 3:
        ...
        var = str(p[2])
        if var.isalnum():
            global_symbol_table[var] = sym_table(var_name = var, var_type = str(p[1]), scope_num = actual_scope + 1)
    ...
    try:
        if (p[1] == '{'):
            scope = scope + 1
            print("opening bracket scope", scope)
        if (p[3] == '}'):
...
def p_local_variable_declaration(p):
    ...
    if len(p) == 3:
        ...
        p[0] = p[2]
        if str(p[2]).isalnum():
            global_symbol_table[str(p[2])] = sym_table(var_name = str(p[2]), var_type = str(p[1]), scope_num = actual_scope)
