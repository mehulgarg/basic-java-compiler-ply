...
def p_class_body(p):
    ...
    global actual_scope
    global current_Node
    global else_flag, current_label, else_label
    ...
    if len(p) == 2 and 'closing_bracket' in a:
        print("closing bracket lolcat")
        actual_scope = actual_scope - 1
        current_Node = current_Node.parent

        if else_flag == 1:
            icg.append(['Label', ' ', ' ', 'L' + str(current_label)])
            for n, i in enumerate(icg):
                if False in i:
                    icg[n] = ['goto', ' ', ' ', 'L' + str(current_label - 1)]
            else_flag = 0
...
def p_classbodydecllist(p):
    ...
    '''
    if len(p) > 2:
        addIntNode("multiple statements in class body",2)

def p_singleclassbody(p):
    '''
...
def p_formal_param_list_opt(p):
    '''
    formal_parameter_list_opt : formal_parameter_list
                              | empty
    '''
    p[0] = p[1]
def p_formalparamlist(p):
    '''
    formal_parameter_list : formal_parameter
                          | formal_parameter_list ',' formal_parameter
    '''
    if len(p) > 2:
        addIntNode("function params",2)
def p_type(p):
    '''type : primitive_type
            | array_type
...
def p_blocks(p):
    ...
    p[0] = p[1]
    if len(p) > 2:
        addIntNode("multi block statements", 2)
...
def p_if_statements(p):
    ...
    '''
    print("IF STATEMENT ONLY")
    global current_label
    icg.append(['Label', ' ', ' ', 'L' + str(current_label -1)])
    addIntNode(p[1], 2)



def p_closing_parantheses_if(p):
    '''
    closing_parantheses_if : ')'
    '''
    global current_label
    temp = temp_stack.pop()
    icg.append(['ifFalse', temp, ' ', 'L' + str(current_label)])
    current_label += 1
    p[0] = p[1]
    print("IF )", p.slice)


def p_if_then_else_statements(p):
    '''
    ...
    '''
    global else_flag, current_label
    for i in p.slice:
        if 'else' == str(i):
...
def p_while_statements(p):
    ...
    '''
    global current_label
    icg.append(['goto', ' ', ' ', 'L' + str(current_label -1)])
    icg.append(['Label', ' ', ' ', 'L' + str(current_label)])
    current_label += 1
    addIntNode(p[1], 2)
...
def p_do_while_statement(p):
    ...
    temp = temp_stack.pop()
    icg.append(['if', temp, ' ', 'L' + str(current_label - 1)])
...
def p_assignment_expression(p):
    ...
    if len(p) > 2:
        ...
        icg.append([p[2], ' ', temp, p[1]])
        addIntNode(p[2], 2)
    print("p_assignment_expr", p.slice)
...
def p_conditional_expression(p):
    ...
    global current_temp
    for i in p.slice:
        if 'name' == str(i) or 'literal' == str(i):
            print("P[1] for expr", p[1], type(p[1]))
            temp_stack.append(p[1])
            break
    p[0] = p[1]
    print(p.slice)
...
def p_ExprBinaryExpr(p):
    ...
    t1 = temp_stack.pop()
    t2 = temp_stack.pop()
    temp = 't' + str(current_temp)
    icg.append([p[2], t2, t1, temp])
    temp_stack.append(temp)
    current_temp += 1
    addIntNode(p[2], 2)
    print("Exp bin slice ", p.slice)
